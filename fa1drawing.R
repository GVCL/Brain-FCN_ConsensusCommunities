draw <- function (f3, Phi = NULL, Path, fe.results = NULL, sort = TRUE, 
    labels = NULL, cut = 0.3, simple = TRUE, errors = FALSE, 
    g = FALSE, digits = 2, e.size = 0.05, rsize = 0.15, side = 2, 
    ...
    ...) 
{
   ...
   if (length(class(fa.results)) > 1) {
    ...
    if (is.null(rownames(factors))) {
        rownames(factors) <- paste(1:nvar, sep = "")
    }
    ...
    for (v in 1:nvar) {
        var.rect[[v]] <- dia.rect(left, top - v - max(0, n.evar - 
            ...
            cex = cex, ...)
        #Rani
        #rowNames = rownames(factors)
        #colNames = colnames(factors)
        #print (head(rowNames))
        #print((colNames))
    }
    ...
    for (f in gf:num.factors) {
        if (pc) {
            ...
            fact.rect[[f]] <- dia.ellipse(left + middle, (num.factors + 
                ...
                #print(colnames(factors)[f])

        }

        for (v in 1:nvar) {
        #### Rani #####          
            if((abs(factors[v, f]) == max(abs(factors[v, 
                ]))) && (abs(factors[v, f]) > cut)){
                
                xnew = round(factors[v, f], digits)
                new <- rbind(new, xnew)
                FacNo <- rbind(FacNo,paste("ML",f,sep=""))
                #print(f)
            }
            ###############
            if (simple && (abs(factors[v, f]) == max(abs(factors[v, 
                ...
                if (pc) {
                  ...
                  dia.arrow(from = fact.rect[[f]], to = var.rect[[v]]$right, 
                   ...
                   #print(fact.rect[[f]])
                   #print( round(factors[v, f], digits))
                }
            }
        }
    ...
    #### Rani #####    
    colnames <- c("Nodes", "FactorWeights","Factors")
    nodes <- rownames(factors)    
    nodesNew <- gsub('V', '', nodes)
    toWrite <- data.frame(nodesNew,new,FacNo)
    #print(toWrite)
    write.table(toWrite, Path, sep = ",", col.names = colnames, row.names = FALSE)
